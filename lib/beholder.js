...
const throttle = process.env.THROTTLE || 2000;
const bigthrottle = process.env.FEED_LIST_THROTTLE || 6000;
...
function compareFeeds(newFeed, oldFeed, cb) {
  ...
  var newArticles = [];
  var tmp = true;
  if (newFeed && oldFeed && !_.isEmpty(oldFeed)) {
...
function emitNewArticles(articles, cb) {
  articles.forEach((art) => {
    log.verbose('storing', art.title);
    feeds.storeNewArticle( art, (err, result) => {
        if (err) return cb(err);
        log.verbose('result of store', result.insertedIds[0]);
        message.sendArticleId(result.insertedIds[0], (err) => {
          if (err) return cb(err);
          return cb(null);
        });
      }
